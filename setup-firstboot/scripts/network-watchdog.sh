...
set_metric() {
    ...
    local ROUTE_LINES
    ROUTE_LINES=$(ip route show dev "$IFACE" 2>/dev/null | grep "^default")
    ...
    while IFS= read -r route; do
        # Remove the old route exactly as it is (including its old metric)
        ip route del $route 2>/dev/null || true
        # Strip the old "metric X" from the string
        local cleaned_route
        cleaned_route=$(echo "$route" | sed -E 's/ metric [0-9]+//g')
        # Add the route back cleanly with the new chosen metric
        ip route add $cleaned_route metric "$METRIC" 2>/dev/null || true
    done <<< "$ROUTE_LINES"
}
